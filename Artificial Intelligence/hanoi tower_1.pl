...
:-
	...
	write('-Megalyteros diskos den mporei na mpei panw apo mikrotero'),nl,
	write('--------------------------'),nl.

%katastasi(passalos,diskos)
arxiki_katastasi:-
retractall(katastasi(_,_)),
assert(katastasi(a,5)),
assert(katastasi(a,4)),
assert(katastasi(a,3)),
assert(katastasi(a,2)),
assert(katastasi(a,1)),
